import type { Locale } from '@/types/database'

const dict = {
  zh: {
    'nav.tasks': '任务',
    'nav.myday': '我的一天',
    'nav.important': '重要',
    'nav.planned': '已计划',
    'nav.newList': '新建列表',
    'nav.renameList': '重命名列表',
    'nav.deleteList': '删除列表',
    'nav.deleteListConfirm': '确定删除此列表及其任务？',
    'task.add': '添加任务',
    'task.steps': '步骤',
    'task.addStep': '添加步骤',
    'task.myDay': '添加到我的一天',
    'task.remind': '提醒',
    'task.due': '截止日期',
    'task.recurrence': '重复',
    'task.recurrence.none': '不重复',
    'task.recurrence.daily': '每天',
    'task.recurrence.weekly': '每周',
    'task.recurrence.weekdays': '工作日',
    'task.note': '备注',
    'task.delete': '删除任务',
    'task.deleteConfirm': '确定删除此任务？',
    'attachment.title': '附件',
    'attachment.add': '添加附件',
    'attachment.uploading': '正在上传…',
    'attachment.delete': '删除附件',
    'attachment.tooLarge': '文件不能超过 10MB',
    'attachment.uploadFailed': '上传失败',
    'share.title': '共享',
    'share.email': '邀请邮箱',
    'share.invite': '发送邀请',
    'share.role.editor': '可编辑',
    'share.role.viewer': '仅查看',
    'share.roleLabel': '角色',
    'share.empty': '暂无成员',
    'share.remove': '移除',
    'share.failed': '邀请失败',
    'share.invites': '邀请',
    'share.noInvites': '暂无待处理邀请',
    'share.accept': '接受',
    'share.decline': '拒绝',
    'nav.invites': '邀请',
    'settings.locale': '语言',
    'settings.theme': '主题',
    'settings.theme.light': '浅色',
    'settings.theme.dark': '深色',
    'task.empty': '暂无任务',
    'task.emptyHint': '在下方输入标题，按回车或点击添加',
    'task.completed': '已完成',
    'task.noList': '默认任务列表未就绪，请稍后重试或检查数据库迁移',
    'task.createFailed': '添加失败，请稍后重试',
    'task.loadFailed': '任务加载失败，请刷新页面',
    'auth.signIn': '登录',
    'auth.signUp': '注册',
    'auth.name': '姓名',
    'auth.email': '邮箱',
    'auth.password': '密码',
    'auth.toggleToSignIn': '已有账号？登录',
    'auth.toggleToSignUp': '没有账号？注册',
    'auth.loading': '加载中…',
  },
  en: {
    'nav.tasks': 'Tasks',
    'nav.myday': 'My Day',
    'nav.important': 'Important',
    'nav.planned': 'Planned',
    'nav.newList': 'New list',
    'nav.renameList': 'Rename list',
    'nav.deleteList': 'Delete list',
    'nav.deleteListConfirm': 'Delete this list and its tasks?',
    'task.add': 'Add a task',
    'task.steps': 'Steps',
    'task.addStep': 'Add a step',
    'task.myDay': 'Add to My Day',
    'task.remind': 'Remind me',
    'task.due': 'Due date',
    'task.recurrence': 'Repeat',
    'task.recurrence.none': 'Does not repeat',
    'task.recurrence.daily': 'Daily',
    'task.recurrence.weekly': 'Weekly',
    'task.recurrence.weekdays': 'Weekdays',
    'task.note': 'Note',
    'task.delete': 'Delete task',
    'task.deleteConfirm': 'Delete this task?',
    'attachment.title': 'Attachments',
    'attachment.add': 'Add attachment',
    'attachment.uploading': 'Uploading…',
    'attachment.delete': 'Delete attachment',
    'attachment.tooLarge': 'File must be 10MB or smaller',
    'attachment.uploadFailed': 'Upload failed',
    'share.title': 'Share',
    'share.email': 'Invite by email',
    'share.invite': 'Send invite',
    'share.role.editor': 'Can edit',
    'share.role.viewer': 'Can view',
    'share.roleLabel': 'Role',
    'share.empty': 'No members yet',
    'share.remove': 'Remove',
    'share.failed': 'Invite failed',
    'share.invites': 'Invites',
    'share.noInvites': 'No pending invites',
    'share.accept': 'Accept',
    'share.decline': 'Decline',
    'nav.invites': 'Invites',
    'settings.locale': 'Language',
    'settings.theme': 'Theme',
    'settings.theme.light': 'Light',
    'settings.theme.dark': 'Dark',
    'task.empty': 'No tasks yet',
    'task.emptyHint': 'Type a title below, then press Enter or Add',
    'task.completed': 'Completed',
    'task.noList': 'Default list is not ready. Retry shortly or apply DB migrations.',
    'task.createFailed': 'Could not add task. Please try again.',
    'task.loadFailed': 'Failed to load tasks. Please refresh.',
    'auth.signIn': 'Sign in',
    'auth.signUp': 'Sign up',
    'auth.name': 'Name',
    'auth.email': 'Email',
    'auth.password': 'Password',
    'auth.toggleToSignIn': 'Already have an account? Sign in',
    'auth.toggleToSignUp': "Don't have an account? Sign up",
    'auth.loading': 'Loading…',
  },
} as const

export type MessageKey = keyof typeof dict.zh

export function t(locale: Locale, key: MessageKey): string {
  return dict[locale][key] ?? dict.en[key] ?? key
}
